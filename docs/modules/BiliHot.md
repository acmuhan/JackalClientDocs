...
初次使用可优先调整：Async、Auto Console、Connect Timeout (s)。
...
- Limit（上限）
 类型：数值；默认：20U
 说明：这是数值型配置。默认值 20U 通常在稳定性与效果之间做了平衡，建议小步调整并观察实际变化。
- Interface（接口）
 类型：枚举；默认："Standard"
 说明：这是选项型配置。默认值 Standard 一般更稳妥；建议按使用场景逐个试用，而不是一次性切换多项。
 可选：Standard（标准）；Mobile（移动端）
- Show Heat Score (Standard)（显示热度分数 (标准)）
 类型：布尔；默认：true
 说明：这是行为开关项。建议先按默认值使用，确认行为符合预期后再逐项启停，避免多个开关同时改动造成排查困难。
- Show Top (Mobile)（显示登顶 (移动端)）
 类型：布尔；默认：true
 说明：这是行为开关项。建议先按默认值使用，确认行为符合预期后再逐项启停，避免多个开关同时改动造成排查困难。
历史更新
